...
const Clientes = () => {
    ...
    const handleEditar = async () => {
        ...
        try {
            await api.put(`/clientes/${clienteParaEditar.id_cliente}`, {
                ...
                razon_social: clienteParaEditar.razon_social,
    ...
    const filtrarClientes = (lista) => {
        ...
        return lista.filter(cliente =>
            (cliente.nombre?.toLowerCase().includes(termino) || false) ||
            (cliente.razon_social?.toLowerCase().includes(termino) || false) ||
    ...
    const renderRows = (clientesLista) => {
        return clientesLista.map((cli) => (
            <Table.Tr key={cli.id_cliente} style={{ opacity: cli.estado === 'inactivo' ? 0.7 : 1 }}>
                ...
                {/* Nombre */}
                <Table.Td>
                    <Text fw={500}>{cli.nombre}</Text>
                    {cli.razon_social && (
                        <Text size="xs" c="dimmed">{cli.razon_social}</Text>
                    )}
                </Table.Td>
        ...
        <Container size="xl" py="xl">
            ...
            >
                <Stack gap="md">
                    ...
                    <TextInput 
                        label="Razón Social" 
                        placeholder="Ej: Empresa S.A. (opcional)"
            ...
            >
                {clienteParaEditar && (
                    <Stack gap="md">
                        ...
                        <TextInput 
                            label="Razón Social" 
                            value={clienteParaEditar.razon_social}
                            onChange={(e) => setClienteParaEditar({
                                ...clienteParaEditar, 
                ...
                {clienteSeleccionado && (
                    <Stack gap="md">
                        <Paper p="md" withBorder>
                            <Group justify="space-between">
                                <div>
                                    <Text fw={700} size="lg">{clienteSeleccionado.nombre}</Text>
                                    {clienteSeleccionado.razon_social && (
                                        <Text c="dimmed" size="sm">{clienteSeleccionado.razon_social}</Text>
                                    )}
